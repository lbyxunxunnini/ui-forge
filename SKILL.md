---
name: ui-forge
description: 面向 App 和 Web UI 的设计工作流 skill。用于新页面设计、重设计、设计评审、设计系统收口和 HTML/CSS/SVG 设计产物生成。优先用在需要从需求理解推进到结构、视觉、交互和输出规范的任务；兼容 uif-、/ui-forge、uid-、/ui-design 等旧触发方式。
---

# UI Forge

`ui-forge` 是一个 **UI 设计工作流 skill**。它不只是给设计意见，还负责把需求收口成可执行的页面结构、交互约束、设计 tokens 和交付产物。

它和 `polanyi-design` 的关系不是替代，而是分层：

- `polanyi-design` 负责高级设计判断，尤其是 gestalt、节奏、密度、视觉语言一致性这类隐性品味判断。
- `ui-forge` 负责把这些判断放进一个可执行流程，产出页面方案、组件边界、设计文档和 HTML/CSS/SVG 结果。

如果任务只是“这个页面为什么丑”“这套视觉哪里像模板”，单独用 `polanyi-design` 就够了。
如果任务要继续推进到重设计、方案分叉、组件抽取、交互说明和交付文件，优先用 `ui-forge`。

## 核心功能

- 智能需求解析 - 自动识别平台、页面类型、设计风格
- 设计诊断与重设计 - 解释为什么不对，再给出结构化修正
...
- 从一句需求开始设计新页面、新模块或新产品界面
- 对现有页面做重设计、视觉诊断、信息层级调整
- 做设计系统收口，明确 tokens、组件、状态和交互规范
- 需要把设计结果交付给后续代码生成或前端实现

不优先使用本 skill 的场景：

- 纯实现问题，例如“这个 CSS 为什么不生效”
- 已有成熟设计系统内的微小样式修复
- 只要一句审美意见，不需要流程、产物或规范
- 纯品牌视觉、插画、logo 方向探索
...
- 诊断模式：分析现有设计哪里失衡，给结构性修正，不直接产出代码
- 设计模式：从需求推进到信息结构、视觉方向、交互和设计规范
- 交付模式：输出 HTML/CSS/SVG、tokens、图标、交互说明和设计指南

是否进入哪种模式，由任务意图决定，不要求用户手动指定。

## 触发场景

当用户主动触发，或任务明显属于 UI 设计工作流时使用本 skill。
...
- "调用ui-forge设计一个登录页"
- "uid-重做这个后台首页，先说哪里像模板"
- "/ui-design 帮我审一下这个组件系统"
