...
当当前模型或运行环境不能读取图片、截图、画布或真实预览时，启动 [输入不完整处理](references/input_incomplete_handling.md) 中的 Visual Understanding Fallback。

- 这是视觉理解能力降级，不是媒体资产降级；不得和 SVG Visual Fallback 混记
- 禁止声称“看到了截图/画面/视觉细节”，只能说明基于文字、文件、DOM/CSS 或用户描述判断
- 若任务依赖截图诊断但没有可读替代信息，必须暂停并请求文字描述、页面结构、设计稿标注或可读取的 HTML/CSS
- 若已有代码或交付文件可读，允许做结构化 DOM/CSS 审查，但视觉结论必须标注 `visual_evidence: text/code-only`
- 视觉审查师无法执行真实缩略/眯眼测试时，必须标注 `视觉未直接验证`，不得把它包装成完整视觉通过
...
- `视觉审查师 → 用户确认`：仅当总分和一票否决项都通过
- `用户确认 → 交付工程师`：仅当用户明确确认设计方案，且本轮目标仍与 brief 对齐
...
22. **视觉理解降级：** 模型或环境无法读取截图/图片/画布时，必须切换到 Visual Understanding Fallback；禁止假装看过视觉输入，禁止输出未标注证据来源的视觉判断。
...
**1. 设计系统锁定**
- 第3个页面设计完成并经用户确认后，tokens.json锁定为项目设计系统
...
对外输出是阶段执法，不是展示偏好。P0：所有对外输出必须使用 `[ui-forge]` 前缀；必须按已闭环阶段顺序输出；任一关键逻辑待确认时停在当前角色。完整合同和标准格式详见 [skill_visibility.md](references/skill_visibility.md)。
