...
def build_card(root: Path, snapshot: dict) -> str:
    ...
    lines.append("")
    lines.append("device_frame_rules:")
    lines.append('  target_device: "iPhone 15"')
    lines.append("  viewport_width: 393")
    lines.append("  viewport_height: 852")
    lines.append('  page_width: "393px"')
    lines.append('  page_height: "852px"')
    lines.append('  page_overflow_y: "auto"')
    lines.append('  status_bar_height: "59px"')
    lines.append('  dynamic_island_size: "126px x 37px"')
    lines.append('  nav_top: "59px"')
    lines.append('  nav_height: "44px"')
    lines.append('  content_top_with_nav: "103px"')
    lines.append('  login_top_padding: "59px"')
    lines.append('  tab_bar_height: "83px"')
    lines.append('  bottom_action_bar_height: "95px"')
    lines.append('  bottom_safe_padding: "34px"')
    lines.append("  home_indicator_required: true")
    lines.append("  gallery_frame_required: true")
    lines.append('  required_gallery_classes: ["iphone-frame", "iphone-screen", "dynamic-island", "status-bar", "home-indicator"]')
    lines.append("")
    lines.append("hierarchy_rules:")
    lines.append('  primary_focal_rule: "每个首屏只能有一个主焦点"')
    lines.append("  focus_chain_required: true")
    lines.append('  squint_test: "页面缩到 25% 后仍能看出主焦点、主内容、主 CTA 的优先级"')
    lines.append('  primary_cta_weight: "主 CTA 必须比次级操作更强"')
    lines.append('  component_differentiation: "不同内容类型的卡片必须有不同扫读结构"')
    lines.append("")
