...
- 同一模块的页面在一组内横向排列
- 不同模块上下排列，整个网页可滚动浏览
- 每个页面有独立的 iPhone 15 手机外框（393×852，除非 design card 明确指定其他设备）
...
App/iOS 输出默认锁定 iPhone 15：

| 项目 | 规格 |
|------|------|
| 页面 viewport | 393×852 |
| 页面容器 | `width: 393px; height: 852px; overflow-y: auto` |
| 真机外壳 | `.iphone-frame`，黑色外壳，圆角 47px |
| 屏幕区域 | `.iphone-screen` |
...
- 各区域尺寸（px 值）
- 安全区域定义
- 设备规范（iPhone 15 画布、状态栏、灵动岛、Home Indicator）
- 响应式断点及适配规则
